    ...
    stages {
            ...
            steps {
                script {
                    withSonarQubeEnv('SonarQube-Server') {
                        sh "sonar-scanner -Dsonar.projectKey=ssp-payment-service -Dsonar.sources=app -Dsonar.login=${SONAR_TOKEN}"
                    }
                }
            }
        }
        ...
        stage('Build and Push Docker Image') {
            steps {
                script {
                    dir('terraform') {
                        sh "terraform init -backend-config=\"bucket=ssp-terraform-state-bucket\" -backend-config=\"key=services/payment-service/terraform.tfstate\" -backend-config=\"region=${AWS_REGION}\""
                    ...
                    def dockerImage = docker.build("ssp-payment-service:${env.BUILD_NUMBER}", ".")
